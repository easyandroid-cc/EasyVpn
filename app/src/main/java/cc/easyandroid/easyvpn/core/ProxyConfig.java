...
public class ProxyConfig {
            ...
            try {
                for (int i = 0; i < m_ProxyList.size(); i++) {
                    try {
                        ...
                        System.out.println("cgp= config start ="+config);
                        InetAddress address = InetAddress.getByName(config.host);
                        if (address != null && !address.equals(config.ServerAddress.getAddress())) {
                            config.ServerAddress = new InetSocketAddress(address, config.port);
                        }
                        System.out.println("cgp= config emd ="+config);
                    } catch (Exception e) {
                    }
                }
            } catch (Exception e) {
    ...
    public String getSessionName() {
        if (m_session_name == null) {
            m_session_name = getDefaultProxy().host;
        }
        ...
        for (String line : lines) {
            ...
            while (m.find()) {
                ...
                if (!m_ProxyList.contains(config)) {
                    ...
                    m_DomainMap.put(config.host, false);
                }
            }
        }
    }
